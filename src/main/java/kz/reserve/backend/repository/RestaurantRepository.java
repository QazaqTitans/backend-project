...
public interface RestaurantRepository extends JpaRepository<Restaurant, Long> {
    ...
    @Query(value = "with all_empty_tables as (\n" +
            ...
            "    )," +
            "stars as (select rt.*, star.average from restaurant rt\n" +
            "    left join (select avg(star) as average, restaurant_id\n" +
            "        from comment group by restaurant_id) star on rt.id = star.restaurant_id) \n" +
            "select * from restaurant rt join stars on stars.id = rt.id where rt.id in (select restaurant_id from all_empty_tables) " +
